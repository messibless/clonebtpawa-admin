...
const EditTennis = () => {
  ...
  if (!fixture) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">No tennis data found</p>
        <button
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.patch(`/tennis/${fixture.id}/`, payload);
      console.log('✅ Match updated:', response.data);

      alert('tennis match updated successfully!');
      navigate('/tennis');
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 max-w-3xl mx-auto">
      <h2 className="text-2xl font-bold mb-6">Edit tennis Match</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          {loading ? 'Updating...' : 'Update tennis Match'}
        </button>
      </form>
    </div>
  );
