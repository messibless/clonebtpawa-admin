...
const TennisList = () => {
  ...
  const fetchFixtures = async () => {
    ...
    try {
      ...
      console.log("tennis data", response.data);
    } catch (err) {
      console.error(err);
      setError('Failed to load tennis fixtures.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this tennis?')) return;
    try {
      await api.delete(`/tennis/${id}/`);
      fetchFixtures();
    } catch (err) {
      console.error(err);
      alert('Failed to delete tennis fixture.');
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h2 className="text-2xl font-bold text-gray-800"> Match Tennis</h2>
        <div className="flex gap-2">
          ...
          <button onClick={() => navigate('/create-tennis')}
      ...
      {fixtures.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <p className="text-gray-500">No tennis available</p>
          <button onClick={() => navigate('/create-tennis')} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Create First tennis</button>
        </div>
      ) : (
            ...
            <div key={fixture.id} className={`relative bg-white rounded-xl shadow-sm hover:shadow-md border-2 ${selectedFixtures.includes(fixture.id) ? 'border-purple-500' : 'border-transparent'}`}>
              ...
              <div className="p-5">
                ...
                <div className="flex gap-2 pt-3 border-t">
                  <button onClick={() => navigate(`/edit-tennis/${fixture.id}`, { state: { fixture } })}
