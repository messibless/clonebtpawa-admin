...
const EditFixture = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('📤 Sending updated football match data:', payload);
      const response = await api.patch(`/football/${fixture.id}/`, payload);
      console.log('✅ Match updated:', response.data);

      alert('Football match updated successfully!');
      navigate('/fixtures');
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 max-w-3xl mx-auto">
      <h2 className="text-2xl font-bold mb-6">Edit Football Match</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          {loading ? 'Updating...' : 'Update Football Match'}
        </button>
      </form>
    </div>
  );
